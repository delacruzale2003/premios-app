...
export default function StoresSidebar({ stores, selectedStore, onSelect, campaignId, refreshStores }: any) {
  ...
  return (
    <div className="flex flex-col h-[500px] lg:h-[700px] bg-zinc-100/50 dark:bg-zinc-900/30 backdrop-blur-xl rounded-[2.5rem] border border-white dark:border-zinc-800/50 p-4 shadow-inner overflow-hidden">
      ...
      <div className="flex gap-2 mb-6 px-1">
        <input 
          className="flex-1 bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800 rounded-2xl px-5 py-3 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 transition-all shadow-sm"
        ...
        {stores.map((s: any) => (
          ...
          >
            ...
            <div className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
              <button onClick={(e) => handleCopyLink(e, s.id)} className="bg-zinc-100 dark:bg-zinc-700 p-2.5 rounded-full hover:bg-blue-500 hover:text-white transition-all">
                {copiedId === s.id ? <Check size={14} strokeWidth={3}/> : <LinkIcon size={14} />}
              </button>
              <button onClick={(e) => deleteStore(e, s.id)} className="bg-zinc-100 dark:bg-zinc-700 p-2.5 rounded-full hover:bg-red-500 hover:text-white transition-all">
                <Trash2 size={14}/>
              </button>
            </div>
          </div>
