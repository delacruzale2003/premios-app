...
export default function StatsView({ campaignId }: { campaignId: string }) {
  ...
  // NUEVO: Detector de móvil para apagar animaciones pesadas
  ...
  return (
    <motion.div 
      // Apagamos la animación de entrada del contenedor en móviles
              ...
              return (
                <div key={name} className="relative w-full group">
                  ...
                  <div className="h-3 w-full bg-zinc-100 dark:bg-black rounded-full overflow-hidden shadow-inner">
                    {/* OPTIMIZACIÓN: Div normal en móvil, motion.div en desktop */}
                    {isMobile ? (
                      <div 
          ...
          {data.hours.map((item: {hour: number, count: number}, i: number) => {
             ...
             return (
              <div key={item.hour} className="flex-1 flex flex-col items-center justify-end h-full group relative">
                ...
                {/* En móviles es mejor no renderizar tooltips complejos de hover */}
                {!isMobile && (
                  <motion.div 
                ...
                {/* OPTIMIZACIÓN: Div estático en móvil, barra animada con física en desktop */}
                {isMobile ? (
                  <div 
                    className="w-full bg-zinc-200 dark:bg-zinc-800 rounded-t-md transition-colors"
