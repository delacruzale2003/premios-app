...
export default function AdminPage() {
  ...
  // NUEVO: Estado para alternar entre el Dashboard de Campañas y el Gestor de Storage
  const [currentView, setCurrentView] = useState<'campaigns' | 'storage'>('campaigns')
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 font-sans text-zinc-900 dark:text-zinc-100 p-4 sm:p-6 lg:p-8 xl:px-12">
      ...
      <div className="w-full max-w-[1920px] mx-auto">
        <header className="mb-6 md:mb-8 border-b border-zinc-200 dark:border-zinc-800 pb-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-3">
               <button 
                ...
                className="text-[10px] font-black uppercase text-zinc-400 hover:text-red-500 transition-colors mr-2"
               >
                 Cerrar Sesión
               </button>

               {/* BOTÓN NUEVO: Alternar entre Campañas y Gestor de Espacio */}
               <button 
                onClick={() => setCurrentView(prev => prev === 'campaigns' ? 'storage' : 'campaigns')}
                className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 hover:scale-105 transition-all shadow-sm active:scale-95 ${
                  currentView === 'storage' 
              ...
              >
                <HardDrive size={14} /> 
                {currentView === 'storage' ? 'Volver a Campañas' : 'Gestor de Espacio'}
              </button>
          ...
          {/* CONTROLES DE CAMPAÑA (Se ocultan si estamos limpiando el storage) */}
          {currentView === 'campaigns' && (
            <div className="flex flex-col xl:flex-row gap-6 items-start xl:items-end justify-between animate-in fade-in duration-300">
              <div>
